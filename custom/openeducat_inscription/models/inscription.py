...
class Inscription(models.Model):
    ...
    academic_year_id = fields.Many2one(
        "op.academic.year", "Periodo académico", required=True
    )
            ...
            if rec.line_ids:
                ...
                if semanal_hour > 30:
                    raise ValidationError(
                        _("No puede inscribir más de 30 horas semanales")
                    )
...
class InscriptionLine(models.Model):
    ...
    subject_id = fields.Many2one("op.session", "Subject", required=True)
    semanal_hour = fields.Integer(
        "Hora semanales", related="name.semanal_hour", store=True
    )
    inscription_id = fields.Many2one(
        "sigu.inscription", "Inscripción", ondelete="cascade"
    )
